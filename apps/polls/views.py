...
def mine(request):
    user = request.user
    page_size = 8
    underway_polls = Poll.objects.filter(end_date__gt=timezone.now(), owner=user)
    finished_polls = Poll.objects.filter(end_date__lt=timezone.now(), owner=user)
    upolls_paginator = Paginator(underway_polls, page_size)
    fpolls_paginator = Paginator(finished_polls, page_size)
    upolls_page = request.GET.get('upage', 1)
    fpolls_page = request.GET.get('fpage', 1)
    upage = upolls_paginator.page(upolls_page)
    fpage = fpolls_paginator.page(fpolls_page)
    return render(request, 'jinja2/polls/mine.html', context=dict(
        upage=upage,
        fpage=fpage,
    ))
...
def detail(request: HttpRequest, poll_id):
    ...
    if request.method == 'GET':
        if request.COOKIES.get('poll_%s' % poll_id):
            return HttpResponse('<h1>您已经参与过给个调查了！</h1>')
        vote_queryset = Vote.objects.select_related('choice').order_by('index')
        poll_questions_queryset = PollQuestion.objects.order_by('index').select_related('question').prefetch_related(
            Prefetch('votes', queryset=vote_queryset, to_attr='voteset'))
...
def vote(request: HttpRequest, poll_id):
    ...
    # print({key: request.META[key] for key in request.META.keys() if key.startswith('HTTP')}, 'meta')
    ...
    if poll.is_open:
        ...
        ret = HttpResponseRedirect(reverse('polls:result', args=(poll_id,)))
        ret.set_cookie('poll_%s' % poll.id, uuid.uuid4().hex, expires=poll.end_date)
        return ret
    else:
        return HttpResponse('<h1>谢谢参与！</h1>')
...
def result(request, poll_id):
    ...
    # print({key: request.META[key] for key in request.META.keys() if key.startswith('HTTP')}, 'meta')
    poll = get_object_or_404(Poll, pk=poll_id)
    if poll.is_open or (request.user and (request.user == poll.owner)):
        vote_queryset = Vote.objects.select_related('choice').order_by('index')
        ...
        return HttpResponse('<h1>该调查不公开结果！</h1>')
...
def submit(request: HttpRequest):
    ...
    try:
        ...
        poll.description = data['desc']
        if data['end_date']:
            poll.end_date = datetime.datetime.strptime(data['end_date'], '%Y-%m-%d')
        if data['is_open'] == '0':
            poll.is_open = False
        poll.save()
        for k in data['questions']:
...
class PollSearchView(SearchView):
    template = 'jinja2/search/search.html'
